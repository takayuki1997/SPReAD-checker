...
- **対象版：公式ツール v1.1（2026年6月版）で通し検証済み（2026-06-22）**。v1.1 の追加点（様式0・2・3・4の英語版対応／手書き署名検出／様式1の氏名ローマ字必須化）はすべて .py 内部ロジックで、殻は無改修のまま完走を確認。次に公式が更新されたら、この節と README「検証の記録」の版表記を更新すること。
...
- **様式の版注意バナー（2026-06-22, commit 18a3943）**：第1回様式での提出（＝多数エラー）に「旧様式かも」と気づけるよう、結果画面に注意を表示。実機で「新様式＝アラート無し／第1回様式＝アラート有り」を利用者が確認済み。
...
- ページ独自 favicon 設定。
- 結果画面に「様式の版」注意バナーを実装（`web/app/index.template.html` の `versionNotice()` / CSS `.vernotice`）。
  - **A（様式0/2/3/4・PDF）**：文科省ツール自身が出す「第2回公募書類を使用しているか＝要確認」をサマリー列・詳細行から拾い、該当ファイル名つきで前面化。**版判定はせず公式出力(`ROUND_RE`)を再掲するだけ**＝「殻は様式知識を持たない」方針を維持（誤検知の概念なし）。
  - **B（様式1・Excel 等、版チェックが無い書類向けの保険）**：`判定済み≥15 かつ ng≥8 かつ (要確認+未記入)/判定済み≥0.7` の全条件成立時のみ、断定しない汎用注意を表示。`未記入`はWARN扱いのため記入途中の正規様式では未発火。
  - 調整は template の `versionNotice()` を直し `node web/build.mjs` で再ビルド。英語版様式の項目名（A の `ROUND_RE`）は実物の英語結果が出たら一語合わせると確実。
